...
| Merge commit subject inspection | `gh api repos/<owner>/<repo>/commits/<sha>` in `apps/decodex/src/github.rs` | Keep `gh api` | Decodex needs the authoritative landed `decodex/commit/1` subject from the merge commit. `gh pr view` does not provide that subject in the current model. |
