...
--expected-turn-id <TURN_ID> --message <TEXT>` and `POST /api/lane-steer`. Both
surfaces require the expected active turn id, audit accepted or rejected state locally,
and keep raw steer text out of public tracker projections.
