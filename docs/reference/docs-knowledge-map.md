...
description: Explains Decodex docs OKF/LLM Wiki navigation, repo-memory evaluation, owner coverage, and where their value appears in this repository.
status: active
...
navigation instead of ordinary prose-only documentation.
...
maintenance anchors, and owner-coverage observations.
...
- Lane indexes guide readers by question type: spec, runbook, reference, decision,
...
structured fields for navigation, graph checks, and drift review.
...
The practical value is not that agents read more files or that a scorer becomes
smarter. The value is that agents can read fewer, better-targeted files and still
preserve source, code, and drift evidence when they update repository knowledge.

## LLM Wiki Value

The LLM Wiki layer is the navigation and maintenance behavior on top of OKF:
...
Those concepts should stay in their owning lanes. This map only provides navigation
edges so graph-based readers can discover them without treating lane indexes as
concept authority.
...
LLM Wiki is most valuable as a navigation discipline. It turns a growing docs tree into
a task-directed graph so agents can start from the smallest relevant concept, follow
...
- Repo-memory evaluation says whether real task questions have reachable owner
...
still being hard to discover. Periodic `decodex docs graph`, index review, and
targeted `decodex docs find` lookups catch that class of issue without pretending to
...
procedures, decisions, and drift-watch points that future agents can navigate.

Out of scope: embeddings, lexical recall, top-N scoring, reranking, and benchmark
hit-rate claims belong to a retrieval/eval system. Such a system may consume OKF and
LLM Wiki artifacts later, but this plugin should stop at the knowledge-bundle and
agent-navigation layers.
