...
| `recover stale-active diagnose` reports blockers. | Preserve the lane. | Inspect the named blockers. Do not clear Linear labels or runtime rows while a run lease, active shared claim, needs-attention label, live process, unknown runtime-marker process liveness, tracked or untracked non-runtime worktree changes, uninspectable worktree, unavailable retained default-branch proof, retained branch commits not reachable from the default branch, private source/review progress evidence, review-policy checkpoint, or PR/review lineage exists. |
