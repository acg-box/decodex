...
- `max_concurrent_agents`
  - type: integer
  - required
  - note: upper-bounds concurrent `decodex` runs per repository; values must be greater than or equal to `1`; Decodex does not apply separate per-state concurrency caps
