...
- Standard: material research must use at least one bounded read-only scout subagent
  or skeptic subagent pass before a recommendation or `decision_ready` claim when
  subagent tools are allowed.
- Deep: architecture, product, cross-boundary, root-cause, or public-contract
  research uses separate scout and skeptic subagents unless the user
  explicitly opts out or subagent tooling is unavailable.

Scale controls cost; the Decision Contract controls readiness.
If subagent tools are unavailable, name the inline fallback and do not describe
it as independent fresh-context review. Subagents stay read-only, receive one
bounded objective, and must not spawn further subagents unless the main thread
