...
pub enum QueryPayload {
	...
	/// Inspect one exact live task and its last reviewer publication receipt.
	...
	/// Discover an account's latest reset operation after a client restart.
	GetAccountResetCardOperation {
...
pub enum EventPayload {
	...
	/// A durable ordinary conversation status item is available for history readback.
	ConversationHistoryChanged {
		/// Exact conversation whose stored history changed.
		conversation_id: EntityId,
	},
...
mod tests {
	...
	fn hello_wire_shape_is_a_stable_json_golden() {
		...
		assert_eq!(
			...
			concat!(
				r#"{"type":"hello","body":{"version":{"major":2,"minor":88},"#,
	...
	fn reset_card_command_has_a_credential_negative_json_golden() {
		...
		assert_eq!(
			serde_json::to_string(&message).unwrap(),
			concat!(
				r#"{"type":"command","body":{"version":{"major":2,"minor":88},"#,
