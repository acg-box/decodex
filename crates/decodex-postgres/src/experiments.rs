use decodex_core::{
	CodexExperimentCommandOutcome, CodexExperimentCreationPossible,
	CodexExperimentIdentity, CodexExperimentObservation, CodexExperimentObservationKind,
	CodexExperimentPrepared, CodexExperimentRejection, CodexExperimentThreadBinding,
};
...
impl FreshCodexExperimentCreation {
	pub fn experiment_id(&self) -> &str { &self.fact.experiment_id }
	pub fn revision(&self) -> i64 { self.fact.revision }
	pub fn attempt_id(&self) -> &str { &self.fact.attempt_id }
	pub fn fenced_at_micros(&self) -> i64 { self.fact.fenced_at_micros }
}
...
impl PostgresStore {
	...
	pub async fn prepare_codex_experiment(
		...
		require_keys(&effect, &[
			"account_id", "account_revision", "build_id", "effect_digest",
			"effect_digest_source", "experiment_id", "managed_run_id",
			"managed_run_revision", "marker", "operation", "prepared_at_micros",
			"repository_cwd", "revision", "role_profile_revision", "routing_snapshot_id",
			"state", "thread_title",
		])?;
	...
	pub async fn mark_codex_experiment_creation_possible(
		...
		let (response,replayed) = self.execute_exact_with_replay_status(
			"SELECT response_bytes,replayed FROM decodex.mark_codex_experiment_creation_possible_exact(\
			 $1,$2,$3::text::uuid,$4,$5::text::uuid)",
			&[&EXACT_COMMAND_PROTOCOL,&idempotency_key,&experiment_id,&expected_revision,&attempt_id],
		).await?;
		let (classification,effect)=parse_envelope(&response,"mark_codex_experiment_creation_possible")?;
		if classification=="stable_domain_rejection" {
			return Ok(CodexExperimentCreationFenceOutcome::Rejected(parse_rejection(&effect)?));
		}
		require_keys(&effect,&["attempt_id","effect_digest","effect_digest_source",
			"experiment_id","fenced_at_micros","operation","revision","state"])?;
		...
		if replayed {
			return Ok(CodexExperimentCreationFenceOutcome::ReplayedAmbiguous {
				experiment_id:experiment_id.to_owned(),revision:2,attempt_id:attempt_id.to_owned(),
			});
		}
		Ok(CodexExperimentCreationFenceOutcome::Fresh(FreshCodexExperimentCreation { fact: CodexExperimentCreationPossible{
			experiment_id:experiment_id.to_owned(),revision:2,attempt_id:attempt_id.to_owned(),
			fenced_at_micros:required_i64(&effect,"fenced_at_micros")?,
		}}))
	}

	/// Bind one exact typed successful creation response. This never searches for or adopts a thread.
	pub async fn bind_codex_experiment_thread(
		&self,idempotency_key:&str,request:&BindCodexExperimentThread,
		...
		if request.expected_revision!=2 || !bounded(&request.thread_id,1024)
			|| !bounded(&request.response_id,1024) || !bounded(&request.response_title,512)
			|| !bounded(&request.response_cwd,4096) || !bounded(&request.response_marker,256) {
			return Err(StoreError::InvalidInput("typed creation response is malformed"));
		}
		let response=self.execute_exact_with_retry(
			"SELECT decodex.bind_codex_experiment_thread_exact($1,$2,$3::text::uuid,$4,\
			 $5::text::uuid,$6,$7,$8,$9,$10,$11)",
			&[&EXACT_COMMAND_PROTOCOL,&idempotency_key,&request.experiment_id,
				&request.expected_revision,&request.attempt_id,&request.thread_id,&request.response_id,
				&request.response_title,&request.response_cwd,&request.response_marker,&request.ephemeral],
		).await?;
		let (classification,effect)=parse_envelope(&response,"bind_codex_experiment_thread")?;
		if classification=="stable_domain_rejection" {
			return Ok(CodexExperimentCommandOutcome::Rejected(parse_rejection(&effect)?));
		}
		require_keys(&effect,&["attempt_id","bound_at_micros","effect_digest",
			"effect_digest_source","experiment_id","marker","operation","response_id",
			"revision","state","thread_id"])?;
		...
		Ok(CodexExperimentCommandOutcome::Applied(CodexExperimentThreadBinding{
			experiment_id:request.experiment_id.clone(),revision:3,
			attempt_id:request.attempt_id.clone(),thread_id:request.thread_id.clone(),
			response_id:request.response_id.clone(),bound_at_micros:required_i64(&effect,"bound_at_micros")?,
		}))
	}

	/// Append one causally bound positive exact observation.
	pub async fn record_codex_experiment_observation(
		&self,idempotency_key:&str,request:&RecordCodexExperimentObservation,
		...
		if request.expected_revision!=3 || !bounded(&request.thread_id,1024)
			|| !bounded(&request.marker,256) || !bounded(&request.source_id,1024)
		...
		let response=self.execute_exact_with_retry(
			...
			&[&EXACT_COMMAND_PROTOCOL,&idempotency_key,&request.experiment_id,&request.expected_revision,
				&request.observation_id,&kind,&request.thread_id,&request.marker,&request.source_id,
				&request.fact_digest],
		).await?;
		let (classification,effect)=parse_envelope(&response,"record_codex_experiment_observation")?;
		if classification=="stable_domain_rejection" {
			return Ok(CodexExperimentCommandOutcome::Rejected(parse_rejection(&effect)?));
		}
		require_keys(&effect,&["effect_digest","effect_digest_source","experiment_id",
			"experiment_revision","fact_digest","kind","marker","observation_id","observed_at_micros",
			"operation","source_id","thread_id"])?;
		if required_str(&effect,"experiment_id")?!=request.experiment_id
		...
		Ok(CodexExperimentCommandOutcome::Applied(CodexExperimentObservation{
			experiment_id:request.experiment_id.clone(),experiment_revision:3,
			observation_id:request.observation_id.clone(),kind:request.kind,
			source_id:request.source_id.clone(),observed_at_micros:required_i64(&effect,"observed_at_micros")?,
		}))
	}
}
...
fn validate_identity(identity:&CodexExperimentIdentity)->Result<(),StoreError>{
	...
	if identity.managed_run_revision<=0 || identity.account_revision<=0
		|| identity.role_profile_revision<=0 || !bounded(&identity.build_id,512)
		|| !bounded(&identity.repository_cwd,4096) || !bounded(&identity.thread_title,512)
...
fn parse_envelope(bytes:&[u8],operation:&str)->Result<(String,Value),StoreError>{
	...
	let effect=document.get("effect").filter(|value|value.is_object())
...
fn verify_digest(effect:&Value)->Result<(),StoreError>{
	...
	projection.remove("effect_digest"); projection.remove("effect_digest_source");
...
fn parse_rejection(effect:&Value)->Result<CodexExperimentRejection,StoreError>{
	...
	let known=match operation {
		"prepare_codex_experiment"=>matches!(code,"invalid_identity"|"lineage_mismatch"|"experiment_exists"),
		"mark_codex_experiment_creation_possible"=>matches!(code,
			"creation_not_authorized"|"attempt_identity_conflict"),
		"bind_codex_experiment_thread"=>matches!(code,
			"typed_response_mismatch"|"thread_identity_conflict"),
		"record_codex_experiment_observation"=>matches!(code,
			"observation_lineage_mismatch"|"observation_identity_conflict"|"observation_fact_conflict"),
		_=>false,
	};
	if !known{return incompatible("stored V15 rejection code is unknown");}
...
fn require_keys(value:&Value,expected:&[&str])->Result<(),StoreError>{
	let object=value.as_object().ok_or_else(||StoreError::Incompatible("stored V15 object is malformed".into()))?;
	let mut actual=object.keys().map(String::as_str).collect::<Vec<_>>(); actual.sort_unstable();
	let mut expected=expected.to_vec(); expected.sort_unstable();
	if actual==expected{Ok(())}else{incompatible("stored V15 object has missing or unknown keys")}
}
...
fn required_i64(value:&Value,key:&str)->Result<i64,StoreError>{
	value.get(key).and_then(Value::as_i64).filter(|number|*number>0)
		.ok_or_else(||StoreError::Incompatible(format!("stored V15 {key} is malformed")))
}
fn validate_uuid(value:&str,label:&'static str)->Result<(),StoreError>{
	if value.len()==36 && value.bytes().enumerate().all(|(index,byte)|match index {
		8|13|18|23=>byte==b'-', _=>byte.is_ascii_digit()||(b'a'..=b'f').contains(&byte),
	}) { Ok(()) } else { Err(StoreError::InvalidInput(label)) }
}
...
fn incompatible<T>(message:&str)->Result<T,StoreError>{Err(StoreError::Incompatible(message.into()))}
