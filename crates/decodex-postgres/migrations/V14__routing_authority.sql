...
-- Zero runtime grants remains valid for migrate-then-provision; no helper or relation is granted.
DO $$
...
BEGIN
	...
	IF anchor_oid IS NULL OR NOT EXISTS (
		...
		RAISE EXCEPTION 'V14 runtime principal anchor is missing or not migration-owned'
			USING ERRCODE='42501';
	END IF;
	SELECT
	...
	IF owner_execute_count<>1 OR runtime_role_count>1 OR invalid_execute_count<>0 THEN
		RAISE EXCEPTION 'V14 runtime principal anchor ACL is ambiguous or unsafe'
			USING ERRCODE='42501';
	END IF;
