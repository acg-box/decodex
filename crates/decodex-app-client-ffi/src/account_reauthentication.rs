...
impl Shared {
	fn new(session_id: String, login_method: LoginMethod) -> Self {
		...
		Self { status: Mutex::new(status), cancel_requested: AtomicBool::new(false) }
	}
...
fn parse_device_code(output: &[u8]) -> Option<String> {
	for suffix_length in [4_usize, 5] {
		...
		for start in 0..=output.len() - code_length {
			...
			if !candidate[..4].iter().all(|byte| byte.is_ascii_uppercase() || byte.is_ascii_digit())
				|| !candidate[5..]
					.iter()
...
mod tests {
	...
	fn pipe_holding_login_manager(session_id: &str) -> (Manager, Arc<Shared>, tempfile::TempDir) {
		let manager = Manager::default();
		let shared = Arc::new(Shared::new(session_id.to_owned(), LoginMethod::BrowserRedirect));
		let worker_shared = Arc::clone(&shared);
