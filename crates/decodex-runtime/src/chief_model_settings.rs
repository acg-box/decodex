	...
	Fut: std::future::Future<Output = Option<Source>>,
{
	...
	let Some(guard) = before.client.history_guard(before.key.history_revision) else {
		return Result::Unavailable;
	};
	let response = before.client.thread_model_settings(&before.key.thread, guard).await;
	if source().await.is_none_or(|after| after.key != before.key) || !owned(store, &before).await {
		return Result::Unavailable;
	}
	...
	let (Ok(work_id), Ok(thread_id), Ok(account_id), Ok(model), Ok(reasoning_effort)) = (
	...
	Result::Available { work_id, thread_id, account_id, model, reasoning_effort }
}
