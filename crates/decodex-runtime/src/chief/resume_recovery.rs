...
impl ChiefCoordinator {
	pub(super) async fn recover_closing_threads(&mut self) -> Result<(), ChiefError> {
		...
		self.recover_persisted_work(item, Some(pending.revision)).await
	}
	...
	pub(super) async fn recover_persisted_work(
		...
		let result = if history_revision.is_some() {
			let Some(guard) = self.client.history_guard(revision) else { return Ok(()) };
			self.client.request_with_history("thread/resume", params, guard).await
		} else {
			self.client.thread_resume(params).await
		};
					...
					&& error.message.starts_with(&format!("thread {thread} is closing;")) =>
			{
				self.closing_resumes.insert(
					...
					ClosingResume {
						...
						turn: turn.clone(),
						revision,
						next: tokio::time::Instant::now() + std::time::Duration::from_secs(60),
					},
				);
				return Ok(());
			},
