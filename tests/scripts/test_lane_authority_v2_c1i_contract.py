...
class LaneAuthorityV2C1IContractTests(unittest.TestCase):
    ...
    def test_p2_rejects_rehashed_rust_receiver_middle_hop_tampering(self):
        resolution_path = Path(
            "tools/lane-authority-inventory/manifests/relations/"
            "rust_receiver_type_resolutions.json"
        )
        binding_path = Path(
            "tools/lane-authority-inventory/manifests/relations/rust_name_bindings.json"
        )
        ...
        resolution = next(
            record
            for record in tampered["records"]
            if record["status"] == "resolved_local_type"
            and len(record["binding_ids"]) >= 2
        )
        replacement = next(
            binding["binding_id"]
            for binding in bindings
        ...
        resolution["binding_ids"][0] = replacement
        resolution["resolution_digest"] = hashlib.sha256(
            self.verifier.canonical_json(
                {
                    key: value
    ...
    def test_p2_rejects_rehashed_rust_method_owner_middle_hop_tampering(self):
        resolution_path = Path(
            "tools/lane-authority-inventory/manifests/relations/"
            "rust_method_owner_resolutions.json"
        )
        binding_path = Path(
            "tools/lane-authority-inventory/manifests/relations/rust_name_bindings.json"
        )
    ...
    def test_p2_rejects_rehashed_rust_qualified_owner_middle_hop_tampering(self):
        resolution_path = Path(
            "tools/lane-authority-inventory/manifests/relations/"
            "rust_qualified_owner_resolutions.json"
        )
        binding_path = Path(
            "tools/lane-authority-inventory/manifests/relations/rust_name_bindings.json"
        )
        ...
        resolution = next(
            record
            for record in tampered["records"]
            if record["status"] in {"resolved_local_module", "resolved_local_type"}
            and len(record["binding_ids"]) >= 2
        )
        replacement = next(
            binding["binding_id"]
