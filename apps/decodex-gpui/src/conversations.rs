...
pub(crate) struct ConversationsSnapshot {
	...
	/// reconciliation does not claim any selected Conversation's feedback surface.
	pub(crate) command_conversation_id: Option<EntityId>,
	...
	pub(crate) refresh: ConversationRefreshState,
	pub(crate) tasks: Vec<ConversationSummary>,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct ConversationLiveDelta {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct QueuedConversationSubmission {
	pub(crate) conversation_id: EntityId,
...
impl Conversations {
	...
	pub(crate) fn snapshot(&self) -> ConversationsSnapshot {
		self.lock().snapshot()
	}
	...
	pub(crate) fn refresh_selected(&self) -> Result<(), ConversationInputError> {
		self.queue_selected_refresh(true)
	}

	/// Re-observe after local history opens without claiming the user-command feedback surface.
	pub(crate) fn refresh_selected_silently(&self) -> Result<(), ConversationInputError> {
		self.queue_selected_refresh(false)
	}
	...
	pub(crate) fn refresh_all(&self) -> Result<(), ConversationInputError> {
		...
		if state.load != ConversationsLoadState::Ready {
			return Err(ConversationInputError::NotReady);
		}
	...
	pub(crate) fn submit(
		...
		let payload = CommandPayload::SubmitConversationTurn {
	...
	pub(crate) fn interrupt(&self) -> Result<(), ConversationInputError> {
		let state = self.lock();
		let task = state.selected_task().ok_or(ConversationInputError::NoSelection)?.clone();
		let turn_id =
	...
	pub(crate) fn command_send_failed(&self, dispatch: &ConversationDispatch) {
	...
	pub(crate) fn command_sent(&self, dispatch: &ConversationDispatch) {
		let Some(command) = dispatch.command() else {
			return;
		};
		let mut state = self.lock();
	...
	pub(crate) fn apply_event(&self, event: &EventEnvelope) {
		...
		if state.selected.is_none()
			&& matches!(
				...
				EventPayload::AccountChanged { .. }
					| EventPayload::AccountLoggedOut { .. }
					| EventPayload::AccountRoutingChanged { .. }
					| EventPayload::AccountRouted { .. }
					| EventPayload::DesktopSettingsChanged { .. }
			) {
			state.clear_catalog();
		}
		...
		match &event.payload {
			...
			EventPayload::ConversationMessageDelta { conversation_id, turn_id, delta } => {
				state.push_delta(ConversationLiveDelta {
	...
	fn apply_command_outcome(
		...
		let batch_refresh = state.refresh_batch.is_some()
			&& matches!(&in_flight.envelope.payload, CommandPayload::RefreshConversation { .. });
...
struct State {
	...
	next_list_cursor: Option<ConversationListCursor>,
	seen_list_cursors: Vec<ConversationListCursor>,
...
impl State {
	...
	fn apply_initial_defaults(&mut self) {
		...
		self.execution = ConversationExecutionSettings {
			...
			fast: resolved.service_tier.as_ref().is_some_and(|tier| tier.as_str() == "priority"),
			service_tier: resolved.service_tier,
		};
		self.initial_defaults_ready = true;
	}
}
...
impl State {
	...
	fn new() -> Self {
		Self {
			...
			execution: ConversationExecutionSettings::new(
				ConversationModel::new("gpt-5.6-sol").expect("default model is valid"),
				ConversationReasoningEffort::High,
				false,
			),
		}
	...
	fn queue_list(&mut self) -> bool {
		...
		let queued = self.queue_query(
			QueryPayload::ListConversations {
				after: after.clone(),
				page_size: ConversationListSize::new(32)
					.expect("constant Conversation page size is valid"),
			},
			ConversationQueryPurpose::List { after },
		);
		if !queued {
	...
	fn queue_routing_successor_source_readback(&mut self, generation: u64) -> bool {
		...
		self.queue_query(
			...
			ConversationQueryPurpose::RoutingSuccessorSource { reconciliation },
		)
	}
	...
	fn queue_query(&mut self, payload: QueryPayload, purpose: ConversationQueryPurpose) -> bool {
		let Some(generation) = self.session.as_ref().map(|session| session.generation) else {
			return false;
		};
		if !self.active || self.pending_query.is_some() || self.in_flight_query.is_some() {
		...
		self.pending_query = Some(PendingQuery {
			envelope: QueryEnvelope {
				version: CURRENT_VERSION,
				query_id: QueryId::new(format!("gpui-conversations/{generation}/{sequence}"))
					.expect("bounded numeric Conversation query identity"),
				payload,
			},
			purpose,
		});
	...
	fn route_list_query_result(
		&mut self,
		generation: u64,
		requested_after: Option<ConversationListCursor>,
		payload: &QueryResultPayload,
	) -> (ConversationRouteOutcome, bool) {
		match payload {
			QueryResultPayload::Conversations(ConversationListResult::Available(page)) => {
				if requested_after
					.as_ref()
	...
	fn advance_refresh_batch(&mut self, archived: bool, failed: bool) -> bool {
		...
		self.refresh = ConversationRefreshState::Refreshing {
	...
	fn accumulate_tasks(&mut self, tasks: Vec<ConversationSummary>) {
		for task in tasks {
	...
	fn snapshot(&self) -> ConversationsSnapshot {
		let command_conversation_id =
			if !self.command_feedback_visible || self.refresh_batch.is_some() {
				None
			} else {
				self.pending_command
					.as_ref()
					.map(|command| command_conversation_id(&command.envelope.payload))
					.or_else(|| {
					...
					.or_else(|| {
						matches!(self.command, ConversationCommandState::OutcomeUnknown)
...
fn accepted_result_task(
	...
	if let (
		CommandPayload::CreateConversationRoutingSuccessor { conversation_id },
		Some(ResultPayload::ConversationRoutingSuccessorAccepted {
			source_conversation_id,
			source_conversation_revision,
			successor,
		}),
	...
	let conversation = match (&in_flight.envelope.payload, result.payload.as_ref()) {
		(
			...
			| CommandPayload::RefreshConversation { .. },
			Some(ResultPayload::ConversationAccepted { conversation }),
		) => conversation,
		(
			CommandPayload::InterruptConversation { .. },
			Some(ResultPayload::ConversationInterruptAccepted { conversation }),
		) => conversation,
		_ => return None,
	};
...
fn accepted_archive_result(
	...
	let command_conversation = match &in_flight.envelope.payload {
		...
		| CommandPayload::RefreshConversation { conversation_id } => conversation_id,
		_ => return None,
	};
	let Some(ResultPayload::ConversationArchived { conversation_id, conversation_revision }) =
...
fn command_identity() -> Result<CommandIdentity, ConversationInputError> {
	...
	Ok(CommandIdentity {
		...
		idempotency_key: IdempotencyKey::new(format!("conversation/{value}"))
...
fn entity_id() -> Result<EntityId, ConversationInputError> {
	EntityId::new(canonical_uuid_v4()?).map_err(|_| ConversationInputError::IdentityUnavailable)
}

fn canonical_uuid_v4() -> Result<String, ConversationInputError> {
...
fn message_text(value: &str) -> Result<HistoryText, ConversationInputError> {
...
fn command_conversation_id(payload: &CommandPayload) -> EntityId {
	match payload {
		...
		| CommandPayload::SubmitConversationTurn { conversation_id, .. }
		| CommandPayload::RefreshConversation { conversation_id }
		| CommandPayload::ArchiveConversation { conversation_id }
		| CommandPayload::InterruptConversation { conversation_id, .. } => conversation_id.clone(),
		_ => unreachable!("Conversations queues only ordinary Conversation commands"),
	}
}
...
fn production_working_directory(
	...
	home: Option<String>,
) -> Option<ConversationWorkingDirectory> {
	match configured {
		Some(configured) => ConversationWorkingDirectory::new(configured).ok(),
		None => home.and_then(|home| ConversationWorkingDirectory::new(home).ok()),
	}
}

fn task_accepts_turn(task: &ConversationSummary) -> bool {
	task.state == ConversationState::Ready
}

fn task_needs_reconciliation(task: &ConversationSummary) -> bool {
	matches!(
		task.state,
		ConversationState::Ready | ConversationState::Running | ConversationState::OutcomeUnknown
	) || task_has_safe_stale_reconciliation(task)
}

fn task_has_safe_stale_reconciliation(task: &ConversationSummary) -> bool {
	matches!(
		(task.state, task.recovery_action),
		(ConversationState::Establishing, Some(ConversationRecoveryAction::ResumeEstablishment),)
			| (
...
fn is_submission_command(payload: &CommandPayload) -> bool {
	matches!(
		payload,
		CommandPayload::CreateConversation { .. } | CommandPayload::SubmitConversationTurn { .. }
	)
}
...
fn task_recovery_command(task: &ConversationSummary) -> Option<CommandPayload> {
	...
	match (task.state, task.recovery_action) {
		...
		(
			ConversationState::EstablishmentPending | ConversationState::Establishing,
			Some(ConversationRecoveryAction::ResumeEstablishment),
		) => Some(CommandPayload::ResumeConversationEstablishment { conversation_id }),
		(
			ConversationState::QuotaExhausted | ConversationState::NoRoute,
			Some(ConversationRecoveryAction::CreateRoutingSuccessor),
		) => Some(CommandPayload::CreateConversationRoutingSuccessor { conversation_id }),
		_ => None,
	}
}

fn task_can_replace(existing: &ConversationSummary, task: &ConversationSummary) -> bool {
...
pub(crate) mod tests {
	...
	) -> Result<ConversationSummary, decodex_protocol::ConversationContractError> {
		ConversationSummary::new(
			conversation_id,
			decodex_protocol::ConversationTitle::new("Test conversation")
	...
	fn complete_list_refresh_removes_a_projection_missing_from_current_authority() {
		...
		let result = QueryResultEnvelope {
			...
			payload: QueryResultPayload::Conversations(ConversationListResult::Available(
				ConversationListPage::new(Vec::new(), None).expect("empty list page is valid"),
			)),
		};
	...
	fn paginated_list_refresh_replaces_only_after_the_complete_readback() {
		let (conversations, server_id, selected) = connected_conversations();
		let first_page_task = conversation_summary(
		...
		let first_result = QueryResultEnvelope {
			...
			payload: QueryResultPayload::Conversations(ConversationListResult::Available(
				ConversationListPage::new(vec![first_page_task.clone()], Some(cursor.clone()))
		...
		let second_query = conversations
			.try_take_dispatch(1, &server_id)
			.and_then(|dispatch| dispatch.query().cloned())
		...
		let second_result = QueryResultEnvelope {
			...
			payload: QueryResultPayload::Conversations(ConversationListResult::Available(
				ConversationListPage::new(vec![selected.clone()], None)
	...
	fn definite_working_directory_failure_can_be_reconciled_or_archived_explicitly() {
		...
		let failed = conversation_summary(
		...
		assert_eq!(
			conversations.submit("this draft must not become a recovery command"),
			Err(ConversationInputError::NotReady)
		);
		assert_eq!(conversations.archive_selected(), Ok(()));
	...
	fn seed_refresh_batch(
		...
		let establishing = conversation_summary(
	...
	fn sidebar_refresh_reconciles_each_provider_thread_then_reloads_the_local_list() {
		...
		assert_eq!(
			conversations.snapshot().refresh,
			ConversationRefreshState::Refreshing { completed: 0, total: 4, archived: 0, failed: 0 }
		);
		...
		let third_result = CommandResultEnvelope {
			...
			payload: None,
			error: Some(CommandError::ConversationRecoveryRequired {
	...
	fn accepted_submission_advances_confirmation_and_requests_exact_history_reload() {
		...
		let result = CommandResultEnvelope {
			...
			payload: Some(ResultPayload::ConversationAccepted { conversation: accepted }),
			error: None,
		};
		...
		assert_eq!(conversations.take_history_reload_request(), None);
	}
	...
	fn archived_submission_rejection_keeps_confirmation_false_and_reloads_authority() {
		...
		let result = CommandResultEnvelope {
			...
			payload: None,
			error: Some(CommandError::ConversationRecoveryRequired {
				action: ConversationRecoveryAction::StartNewConversation,
			}),
		};
		...
		assert_eq!(
			snapshot.command,
			ConversationCommandState::ManualRecovery(
				ConversationRecoveryAction::StartNewConversation
			)
		);
		assert_eq!(conversations.take_history_reload_request(), Some(task.conversation_id));
		assert!(matches!(
			...
			Some(ConversationDispatch::Query(QueryEnvelope {
				payload: QueryPayload::ListConversations { .. },
				..
			}))
		));
	}
	...
	fn sent_command_disconnect_requires_readback_before_explicit_retry() {
			...
			.expect("reconnect queues authoritative list readback")
		{
			...
			ConversationDispatch::Command(_) => panic!("unknown outcome must not auto-resubmit"),
		};
		let result = QueryResultEnvelope {
			version: CURRENT_VERSION,
			server_id: server_id.clone(),
			query_id: query.query_id,
			payload: QueryResultPayload::Conversations(ConversationListResult::Available(
				ConversationListPage::new(vec![task.clone()], None).expect("test page is valid"),
			)),
		};
	...
	fn send_failure_requires_readback_before_explicit_retry() {
		...
		{
			ConversationDispatch::Query(query) => query,
			ConversationDispatch::Command(_) => panic!("failed send must not auto-resubmit"),
		};
		let result = QueryResultEnvelope {
			version: CURRENT_VERSION,
			server_id: server_id.clone(),
	...
	fn waiting_recovery_replaces_the_archived_source_with_its_successor() {
		...
		let result = CommandResultEnvelope {
			...
			payload: Some(ResultPayload::ConversationRoutingSuccessorAccepted {
	...
	fn routing_successor_lost_response_reconnect_selects_exact_redirect_and_removes_archived_source()
	 {
		...
		let list_result = QueryResultEnvelope {
			...
			query_id: list.query_id,
			payload: QueryResultPayload::Conversations(ConversationListResult::Available(
				ConversationListPage::new(vec![successor.clone()], None)
		...
		let successor_result = QueryResultEnvelope {
			...
			payload: QueryResultPayload::Conversation(ConversationResult::Available(
				successor.clone(),
			)),
		};
		assert_eq!(
