...
impl ChiefSurface {
	pub(super) fn invalidate_live_reviewer_for_snapshot(&mut self, next: &ChiefSnapshotDto) {
		...
		let unchanged = same_source
			&& matches!((before,after),(Some(a),Some(b)) if a.codex_thread_id==b.codex_thread_id && a.active_turn_id==b.active_turn_id && b.dispatch_state==ChiefDispatchStateDto::Running);
		if !unchanged {
