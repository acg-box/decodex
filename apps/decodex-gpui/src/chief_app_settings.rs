...
mod tests {
	...
	fn account_settings_require_review_and_clear_on_disconnect(cx: &mut gpui::TestAppContext) {
		...
		surface.update(visual,|s,_| {
			...
			s.request=Some(ChiefRequestResult::Available {event_id:7,work_id:"root".into(),method:"mcpServer/elicitation/request".into(),request_json:decodex_protocol::HistoryText::new(json!({"serverName":"codex_apps","mode":"form","message":"Review action","requestedSchema":{"type":"object","properties":{}},"_meta":{"connector_id":"calendar","link_id":"work","codex_approval_kind":"tool_call"}}).to_string()).unwrap()});
		});
		visual.update(|w, cx| {
