...
impl ChiefSurface {
	...
	pub(super) fn invalidate_app_settings(&mut self, next: &ChiefSnapshotDto) {
		let Some(event) = self.app_settings.event else { return };
		let Some(work) = self.selected.as_ref() else {
		...
		if self.snapshot.as_ref().is_none_or(|s| s.runtime_source != next.runtime_source)
			...
			|| !next.pending_events.iter().any(|e| e.id == event && &e.work_item_id == work)
		{
			self.reset_app_settings();
		}
	}
	...
	fn update_account_settings(&mut self, event: i64, edit: Option<Edit>, cx: &mut Context<Self>) {
		...
		if *event_id != event
			...
			|| (edit.is_some() && !self.app_settings.reviewed)
		{
			return;
		}
		...
		let action = match edit {
			Some(edit) => {
				...
				let Some(State::Available { review_token, can_update: true, .. }) =
		...
		self.app_settings.reviewed = false;
		...
		self.app_settings.task=Some(cx.spawn(async move |surface,cx| {
			...
			let _=surface.update(cx,|s,cx| {
                ...
                s.app_settings.reviewed = !saving;
				let (state,outcome)=result.unwrap_or((State::Unavailable,None));
				s.app_settings.feedback=match outcome {
					Some(Ok(ChiefCommandResponse::Accepted {..})) => "Native save acknowledged. Tool and managed policies can still override this connection setting.",
					Some(Ok(ChiefCommandResponse::Rejected {..})) => "The edit was not accepted. Review the current settings before changing them.",
					Some(_) => "The edit may have been saved. Inspect the configuration and receipt below; it will not be retried automatically.",
		...
		if current && !busy {
			...
			}) = &self.app_settings.state
			{
				panel = panel
					...
					.child(
						"Select an override to save it. This does not answer the pending approval. Tool and managed policy take precedence.",
					);
				panel = panel.child(format!("Shared configuration: {config_file}"));
				if let Some(receipt) = last_edit {
					panel = panel.child(format!(
						"{}: {}. Original task: {}; Codex account: {}.",
						receipt.target, receipt.outcome, receipt.work_id, receipt.account_id
					));
				}
				if !can_update || !self.app_settings.reviewed {
					return panel
						.child(
							"Read current settings to review another edit. Unconfirmed writes must be resolved first.",
						)
						.into_any_element();
				}
...
mod tests {
	...
	fn account_settings_require_review_and_clear_on_disconnect(cx: &mut gpui::TestAppContext) {
		...
		surface.update(visual,|s,_| {
			...
			s.request=Some(ChiefRequestResult::Available {event_id:7,work_id:"root".into(),method:"mcpServer/elicitation/request".into(),request_json:decodex_protocol::ChiefRequestText::new(json!({"serverName":"codex_apps","mode":"form","message":"Review action","requestedSchema":{"type":"object","properties":{}},"_meta":{"connector_id":"calendar","link_id":"work","codex_approval_kind":"tool_call"}}).to_string()).unwrap()});
		});
		visual.update(|w, cx| {
