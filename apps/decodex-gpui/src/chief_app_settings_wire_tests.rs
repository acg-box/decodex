...
fn real_settings_click_reads_then_sends_once_and_refreshes_unknown_result(
  ...
  s.request=Some(ChiefRequestResult::Available {event_id:7,work_id:"root".into(),method:"mcpServer/elicitation/request".into(),request_json:HistoryText::new(json!({"serverName":"codex_apps","mode":"form","message":"Review","requestedSchema":{"type":"object","properties":{}},"_meta":{"connector_id":"calendar","link_id":"work"}}).to_string()).unwrap()});
 });
	visual.update(|w, cx| {
