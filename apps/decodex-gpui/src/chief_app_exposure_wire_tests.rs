...
async fn serve(listener: tokio::net::UnixListener) -> Vec<ChiefActionDto> {
	...
	for index in 0..3 {
		...
		if index == 1 {
			...
			let ChiefActionDto::SetAppToolExposure { work_id, connector_id, review_token, omit } =
				&*action
			else {
		...
		let QueryPayload::GetChiefAppExposure { work_id, connector_id } = query.payload else {
			panic!("account query")
		};
		assert_eq!(work_id.as_str(), "root");
		assert_eq!(connector_id.as_str(), "calendar");
		...
		let result = ServerMessage::QueryResult(QueryResultEnvelope {
			...
			payload: QueryResultPayload::ChiefAppExposure(state),
		});
