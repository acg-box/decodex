...
#[derive(Debug)]
enum Event {
	Message { from: usize, to: usize, text: String, dashed: bool, arrow: char },
	Open(String),
	Branch(String),
	Close,
	Note { left: usize, right: usize, text: String },
}
...
pub(super) fn render(body: &[&str], max_width: usize) -> Result<Vec<Vec<Span>>, RenderError> {
	...
	for &line in body {
		let mut rest = line;
		if let Some(after) =
			line.strip_prefix("participant ").or_else(|| line.strip_prefix("actor "))
		{
			rest = after;
			...
			line.split_once(' ')
		{
			...
			line.strip_prefix("Note over ").or_else(|| line.strip_prefix("note over "))
		{
			rest = after;
	...
	let name_width = people.iter().map(|(_, label, _)| label.width()).max().unwrap_or(0);
	...
	let centers = (0..people.len()).map(|i| 10 + box_width / 2 + stride * i).collect::<Vec<_>>();
	...
	for event in events {
		let count =
			if matches!(event, Event::Message { from, to, .. } if from == to) { 4 } else { 3 };
		let top = rows.len();
		...
		match event {
			Event::Message { from, to, text, dashed, arrow } => {
	...
	if rows
		.iter()
		.any(|row| row.iter().rposition(|cell| cell.symbol != ' ').is_some_and(|x| x >= max_width))
	{
