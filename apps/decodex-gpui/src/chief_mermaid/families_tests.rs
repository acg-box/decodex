...
#[test]
fn er_cardinalities() {
	for (left, source_card) in [("||", "1"), ("|o", "0..1"), ("}|", "1..many"), ("}o", "0..many")] {
...
fn family_limits() {
	for source in [
		format!(
			"sequenceDiagram; {}",
			(0..9).map(|i| format!("participant P{i};")).collect::<String>()
		),
		format!("sequenceDiagram; {}", "A->>B: msg;".repeat(65)),
		format!("sequenceDiagram; {} A->>B: msg; {}", "loop retry;".repeat(5), "end;".repeat(5)),
...
fn sequence_arrows_preserve_sender_recipient_and_style() {
	for (operator, forward, reverse, dashed) in [
		...
		for (from, to, tip) in [("A", "B", forward), ("B", "A", reverse), ("B", "B", reverse)] {
			...
			let a =
				rows[1].iter().position(|ch| *ch == 'A').expect("valid upstream diagram fixture");
			let b =
...
fn canvas_limit_applies_even_with_unlimited_caller_width() {
	let graph = format!("graph LR; {}", format!("A-->|{}|B;", "x".repeat(40)).repeat(24));
	let sequence = format!(
		"sequenceDiagram; {} {}",
