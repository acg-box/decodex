...
pub(super) fn render(graph: &Graph, max_width: usize) -> Result<Vec<Vec<Span>>, RenderError> {
	...
	let box_cross = if horizontal {
		...
		labels.iter().flatten().map(|label| label.width()).max().unwrap_or(0) + 4
	};
	...
	let across =
		if graph.edges.is_empty() { box_cross } else { first_lane + graph.edges.len() * 2 - 1 };
	let (width, height) = if horizontal { (along - 2, across) } else { (across, along - 2) };
	if width > max_width {
	...
	for (i, lines) in labels.iter().enumerate() {
		...
		for (j, line) in lines.iter().enumerate() {
			let (x, y) = if horizontal { (start + 2, j + 1) } else { (2, start + j + 1) };
	...
	for (i, edge) in graph.edges.iter().enumerate() {
		...
		for y in source.min(target) + 1..source.max(target) {
			canvas.set(first_lane + 2 * i, y, Cell::edge(if edge.dashed { '┆' } else { '│' }));
		}
	}
	for (i, edge) in graph.edges.iter().enumerate() {
		let (source, target) = endpoints[i];
		...
		canvas.set(lane, source, Cell::edge(if source < target { '┐' } else { '┘' }));
		canvas.set(lane, target, Cell::edge(if source < target { '┘' } else { '┐' }));
		for (port, label) in [(source, &edge.label), (target, &edge.target_label)] {
			let (x, y) = if horizontal { (port + 1, box_cross + 1) } else { (box_cross + 2, port) };
