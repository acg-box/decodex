...
fn fixture(
	...
	let thread = std::thread::spawn(move || {
		...
		runtime.block_on(async {
			...
			tokio::time::timeout(std::time::Duration::from_secs(15), serve(listener, generate))
				.await
				.unwrap()
		})
	});
...
async fn serve(listener: tokio::net::UnixListener, generate: bool) -> Vec<ChiefActionDto> {
...
fn lost_recap_reply_is_read_back_and_cancelled_by_exact_request_without_replay() {
	let (_root, profile, server) = fixture(true);
	let runtime = tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap();
	runtime.block_on(async {
...
fn opening_a_cold_recap_only_reads_and_does_not_start_inference() {
	let (_root, profile, server) = fixture(false);
	let runtime = tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap();
	runtime.block_on(async {
