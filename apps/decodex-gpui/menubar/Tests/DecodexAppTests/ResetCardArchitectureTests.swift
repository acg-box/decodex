...
final class ResetCardArchitectureTests: XCTestCase {
	...
	func testMenuBarOwnerDisablesAutomaticAndSuddenTermination() throws {
		...
		XCTAssertTrue(source.contains("final class DecodexMenuBarHost"))
		XCTAssertTrue(source.contains("prepareForTermination"))

		let packageURL = sourceURL
			.deletingLastPathComponent()
			.deletingLastPathComponent()
			.deletingLastPathComponent()
		let repositoryURL = packageURL
			.deletingLastPathComponent()
			.deletingLastPathComponent()
			.deletingLastPathComponent()
		let info = try String(
		...
		XCTAssertTrue(info.contains("<key>NSSupportsAutomaticTermination</key>"))
		XCTAssertTrue(info.contains("<key>NSSupportsSuddenTermination</key>"))
	}
	...
	func testBundleStagesSwiftMenuBarNativeClientAndDaemonWithoutSecondApp() throws {
		let testsURL = URL(fileURLWithPath: #filePath)
			.deletingLastPathComponent()
		let appURL =
			testsURL
		...
		XCTAssertTrue(script.contains(#"NATIVE_CLIENT_LIBRARY="libdecodex_app_client_ffi.dylib""#))
		XCTAssertTrue(script.contains(#"MENU_BAR_LIBRARY="libDecodexMenuBar.dylib""#))
		...
		XCTAssertTrue(
			stageTest.contains("libDecodexMenuBar.dylib")
		)
		XCTAssertTrue(
		...
		for retiredPackageTerm in ["-p decodex-cli", "DecodexMenuBar.app", ":8192"] {
			XCTAssertFalse(
				...
				"Build script still packages a retired UI or CLI component \(retiredPackageTerm)."
			)
		}
	}
	...
	func testGPUIBundleBuildsAndEmbedsTheMenuBarFromTheSameCheckout() throws {
		let testsURL = URL(fileURLWithPath: #filePath)
			.deletingLastPathComponent()
		let packageURL = testsURL.deletingLastPathComponent().deletingLastPathComponent()
		let repositoryURL = packageURL
			.deletingLastPathComponent()
			.deletingLastPathComponent()
			.deletingLastPathComponent()
		...
		let bridge = try String(
			contentsOf: repositoryURL.appendingPathComponent(
				"apps/decodex-gpui/src/native_menu_bar.rs"
			),
			encoding: .utf8
		)

		XCTAssertTrue(script.contains(#"MENU_BAR_PACKAGE="$ROOT/apps/decodex-gpui/menubar""#))
		XCTAssertTrue(script.contains(#"cp "$SWIFT_BIN/$MENU_BAR_LIBRARY" "$FRAMEWORKS/$MENU_BAR_LIBRARY""#))
		XCTAssertFalse(script.contains("DecodexMenuBar.app"))
		XCTAssertFalse(script.contains("Library/LoginItems/"))
		XCTAssertTrue(bridge.contains("libDecodexMenuBar.dylib"))
		XCTAssertTrue(bridge.contains("libc::dlopen"))
		XCTAssertTrue(bridge.contains("decodex_menu_bar_set_visible"))
	}
	...
	func testLoginRecoveryUsesNativeBrowserOAuthAndNoPanelDisappearCancel() throws {
		...
		XCTAssertTrue(view.contains(#"Text("Browser")"#))
		XCTAssertTrue(view.contains(#"Text("Device code")"#))
		XCTAssertFalse(view.contains("Continue in browser"))
		XCTAssertFalse(view.contains("Use device code"))
		XCTAssertTrue(view.contains("presentation.headerAccountLabel"))
