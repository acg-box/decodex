...
fn merged_worktree_cleanup_debts_detects_dirty_merged_worktree() {
	...
	tests::run_git(&worktree_path, &["add", "README.md"]);
	tests::run_git(&worktree_path, &["commit", "-m", "feature work"]);
	tests::run_git(
		&repo_root,
	...
	let debts = worktree::merged_worktree_cleanup_debts(&repo_root, &worktree_root, "main")
...
fn merged_worktree_cleanup_debts_treats_decodex_runtime_artifacts_as_clean() {
	...
	fs::write(worktree_path.join(RUN_ACTIVITY_MARKER_FILE), "agent_run\n")
...
fn merged_worktree_cleanup_debts_ignores_dirty_worktree_started_from_old_default() {
	...
	tests::run_git(&repo_root, &["add", "main.txt"]);
	tests::run_git(&repo_root, &["commit", "-m", "advance main"]);
...
fn merged_worktree_cleanup_debts_ignores_dirty_worktree_at_default_tip() {
	...
	fs::write(worktree_path.join(RUN_ACTIVITY_MARKER_FILE), "started\n")
