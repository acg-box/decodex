...
pub(super) fn mcp_test_fixture_control_evidence(
...
pub(super) fn private_events_are_cleanup_audit_evidence(events: &[PrivateExecutionEvent]) -> bool {
	!events.is_empty() && events.iter().all(private_event_is_cleanup_audit)
}

pub(super) fn private_event_is_cleanup_audit(event: &PrivateExecutionEvent) -> bool {
...
pub(super) fn mcp_test_fixture_allowed_live_blocker(blocker: &str) -> bool {
	matches!(
		blocker,
