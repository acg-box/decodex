use super::{
	AGENT_HANDOFF_INDEX_SCHEMA, AgentEvidenceFileWriteContext, AgentEvidenceProjectView,
	...
	RUNS_DIR_NAME, Result, agent_connector_backoff, agent_evidence_project_ids,
	agent_recovery_contract, agent_recovery_worktree, build_agent_blockers, build_run_capsules,
	current_month_bucket, current_timestamp, run_capsule_ref, runtime, write_agent_evidence_files,
};
...
pub(in crate::orchestrator) fn write_agent_evidence_snapshot(
	...
	for project_id in agent_evidence_project_ids(snapshot) {
		...
		let mut run_capsules =
			build_run_capsules(&project_view, &generated_at, &runs_dir, &month_bucket);
