mod program_reconciler {
	...
	fn records_private_program_dispatch_selection_event_with_node_provenance() {
		let (_temp_dir, config, workflow) = temp_project_layout();
		let store = StateStore::open_in_memory().expect("state store should open");
		let issue = program_reconciler_issue("issue-ready-event", "PUB-202", "Todo", &[]);
		...
		let selected = selection.selected.expect("ready node should be selected");
		let program_dispatch =
			selected.program_dispatch.expect("selection should carry provenance");
	...
	fn live_reconciliation_clears_missing_orphaned_mapping_before_program_selection() {
		...
		assert!(blocked.selected.is_none());
		assert!(
			store.worktree_for_issue(&issue.id).expect("worktree lookup should succeed").is_some()
		);
		...
		assert!(
			store.worktree_for_issue(&issue.id).expect("worktree lookup should succeed").is_none()
		);
