...
impl LoopScenarioHarness {
	...
	fn record_review_guardrail_and_assert_harness_feedback(
		&self,
	...
	fn record_review_churn_escalation(&self) {
		...
		assert_eq!(
			ReviewPolicyStopReason::Exhausted.error_class(),
...
fn loop_scenario_assert_phase_goal_completion_runs_validation() {
	...
	let (_temp_dir, config, workflow) =
		super::temp_project_layout_with_workflow_markdown(&workflow_markdown);
	let issue =
		super::sample_issue("In Progress", &[tracker::automation_active_label(LOOP_SCENARIO_GATE_SERVICE_ID).as_str()]);
...
fn loop_scenario_assert_validation_guardrail_stops_after_threshold() {
	let (_guardrail_temp_dir, guardrail_config, _guardrail_workflow) = super::temp_project_layout();
	let guardrail_store = StateStore::open_in_memory().expect("guardrail store should open");
	let guardrail_issue = super::sample_issue("In Progress", &[]);

	for round in 1..=2 {
		let guardrail_issue_run =
			super::loop_guardrail_issue_run(&guardrail_config, &guardrail_issue, round);
		...
		assert!(
			stop.is_none(),
			"round {round} should keep repairing before the threshold"
		);
	}

	let guardrail_issue_run = super::loop_guardrail_issue_run(&guardrail_config, &guardrail_issue, 3);
	let stop = orchestrator::retryable_failure_loop_guardrail_stop(
		&guardrail_config,
		&guardrail_store,
		&guardrail_issue_run,
...
fn loop_scenario_review_payload(
	nonclean_rounds: i64,
