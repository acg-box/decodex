...
fn authority_boundary_public_api_surface_requires_enhanced_evidence() {
	...
	assert_eq!(
		event.payload()["changed_surfaces"][0]["surface"],
		"public_api"
	);
}
...
fn authority_boundary_surface_policy_matrix_classifies_risk() {
	...
	assert!(
		AuthorityBoundaryPolicyDecision::RequiresEnhancedEvidence
			.allows_autonomous_recovery()
	);
	assert!(AuthorityBoundaryPolicyDecision::BlockLanding.allows_autonomous_recovery());
	assert!(
		!AuthorityBoundaryPolicyDecision::RequiresHumanDecision.allows_autonomous_recovery()
	);
	assert!(
		AuthorityBoundaryPolicyDecision::RequiresEnhancedEvidence.requires_enhanced_evidence()
	);
