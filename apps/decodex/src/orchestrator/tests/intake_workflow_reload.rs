...
pub(super) fn expected_developer_instructions(
	...
	sections.push(String::from(
			"Docs impact contract\n- Before any terminal finalize path, classify docs impact as `none`, `update_required`, `research_required`, or `drift_required`, and record it in a current-HEAD `issue_progress_checkpoint` as `docs_impact`.\n- Decodex records internal validation evidence for phase transitions; do not create checkpoint comments solely to satisfy phase ceremony.\n- If behavior, commands, config, schemas, status, validation, workflow, or docs changed, update the owning Decodex page under `docs/` when the changed claim is Decodex product or runtime authority.\n- If authority is missing or contradictory, use `research_required` and route the investigation through the external installed team research workflow; only accepted results become Decodex docs updates.\n- Run the repository validation gate that matches the touched surface.\n- Treat touched-surface documentation or semantic-drift failure as a completion blocker. Fix issue-local failures in the lane. Do not route pre-existing, repo-wide, or global-baseline failures through `manual_attention`; record the blocker in private evidence and let Decodex retain or isolate the baseline lane.",
	));
	sections.push(String::from(
