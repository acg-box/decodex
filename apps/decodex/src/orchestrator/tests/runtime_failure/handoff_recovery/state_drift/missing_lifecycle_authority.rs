...
fn handle_failure_requires_rebind_when_clean_handoff_checkpoint_has_no_marker() {
	...
	.expect("missing lifecycle authority should require explicit rebind attention");
	...
	assert!(
		state_store
			...
			.is_none(),
		"missing lifecycle authority must not be reclassified as no effective diff"
	);
	...
	assert!(events.iter().any(|event| {
		event.event_type() == "review_handoff_state_drift_detected"
			&& event.payload()["reason"] == "missing_review_lifecycle_record"
