...
use crate::{
	...
	orchestrator::types::operator_status::execution_program::{
		identifiers,
		node::{self, OperatorExecutionProgramNodeStatus},
	},
...
impl OperatorExecutionProgramStatus {
	pub(crate) fn from_summary(
		...
		Self {
			...
			node_readbacks: evaluation
				...
				.iter()
				.filter(|program_node| {
					node::operator_execution_program_node_should_render(program_node)
				})
				.map(node::operator_execution_program_node_readback)
				.collect(),
	...
	pub(crate) fn missing_contract(record: &ExecutionProgramRecord) -> Self {
		...
		Self {
			...
			mapped_issue_identifiers:
				identifiers::operator_execution_program_mapped_issue_identifiers(record),
			node_readbacks: node::operator_execution_program_missing_contract_nodes(record),
