...
mod tests {
	use crate::orchestrator::{
		LaneNextAction,
		lane_decision::{
			self, IssueDispatchMode, LaneDecisionSnapshot, OwnedLaneAction, PhaseGoalKind,
			RepoGateFailureDisposition, RetryKind,
		},
	};
	...
	fn phase_acceptance_failure_projects_kernel_retry_to_legacy_retry_failure() {
		...
		let decision = lane_decision::decide_lane_next_action(&snapshot);
