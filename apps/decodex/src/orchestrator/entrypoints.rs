...
struct StatusSnapshotCacheMiss {
	reason: String,
	next_action: String,
}

struct StatusSnapshotHttpResponse {
	body: Vec<u8>,
	published_at_unix_epoch: Option<i64>,
}
...
fn status_snapshot_from_local_operator_cache(
...
fn status_snapshot_from_operator_cache_response(
	project: &ServiceConfig,
	limit: usize,
	...
	let snapshot = serde_json::from_slice::<OperatorStatusSnapshot>(&response.body).map_err(|error| {
		status_snapshot_cache_miss(
...
fn parse_operator_snapshot_http_response(
	...
	if body.is_empty() || body.as_slice() == b"{}" {
		return Err(String::from("local operator listener has not published a status snapshot yet"));
	}
...
fn project_status_snapshot_from_operator_cache(
	...
	let Some(project_status) = snapshot
		.projects
		.iter()
		.find(|status| status.project_id == project.service_id())
		.cloned()
	else {
		return Err(status_snapshot_cache_miss(
			format!(
				"operator snapshot does not include project `{}`",
				project.service_id()
			),
...
fn add_status_snapshot_cache_miss_warning(
	snapshot: &mut OperatorStatusSnapshot,
	project: &ServiceConfig,
