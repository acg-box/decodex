...
use crate::orchestrator::{
	OperatorHistoryLedgerOutcome, OperatorHistoryLedgerRecord, status_history_ledger::records,
};
...
pub(crate) fn operator_history_ledger_outcome(
	...
	let Some(final_record) = final_record::final_history_ledger_record(records) else {
		return execution_log_history_ledger_outcome(records);
	};
	let ledger_status =
...
fn execution_log_history_ledger_outcome(
	records: &[OperatorHistoryLedgerRecord],
) -> OperatorHistoryLedgerOutcome {
	let latest = records
		.iter()
	...
	OperatorHistoryLedgerOutcome {
		ledger_status: String::from("partial"),
		final_outcome: String::from("execution_log"),
		final_event_type: latest.map(|entry| entry.record.event_type.clone()),
		final_event_at: latest.map(|entry| entry.record.event_timestamp.clone()),
		summary: latest.map(|entry| {
			format!(
				"Execution ledger has {} records; latest event is `{}`. Final lifecycle state is controlled by runtime authority records.",
				records.len(),
