...
use crate::{
	orchestrator::{
		Command, EXTERNAL_REVIEW_ACK_TIMEOUT_SECS, EXTERNAL_REVIEW_MERGE_VISIBILITY_TIMEOUT_SECS,
		EXTERNAL_REVIEW_REQUEST_BODY, ExternalReviewRequestCiGate,
		GhPullRequestReviewStateInspector, HashMap, HashSet, IssueDispatchMode, IssueRunPlan,
		IssueTracker, OffsetDateTime, Path, PostReviewLaneClassification, PostReviewLaneDecision,
		PostReviewLaneSnapshot, PostReviewLaneStateLoad, PostReviewRuntimeState,
		PullRequestReviewState, PullRequestReviewStateInspector, Report, Result,
		RetainedReviewLaneBlocked, RetainedReviewLaneLoad, RetainedReviewNeedsAttention,
		RetainedReviewRunIdentity, ReviewHandoffMarker, ReviewOrchestrationMarker,
		ReviewOrchestrationPhase, ServiceConfig, StateStore, TerminalFailureWritebackRuntime,
		TrackerIssue, WorkflowDocument, WorktreeMapping, WorktreeSpec, eyre, github,
		kernel::{
			command::{CommandFact, CommandIntent, CommandIntentKind},
			post_review,
		},
	},
	state::{self, WORKTREE_PROVENANCE_RUNTIME_RECORDED},
	tracker,
};
...
pub(crate) fn worktree_mapping_is_stale_terminal_local_residue(
	...
	if active_issue_ids.contains(mapping.issue_id())
		|| !super::looks_like_tracker_issue_identifier_key(mapping.issue_id())
	...
	Ok(super::local_run_attempt_status_is_terminal(attempt.status()))
}
	...
	I: PullRequestReviewStateInspector,
{
	...
	for worktree in worktrees {
		...
		if let Some(reason) = super::validate_review_orchestration_marker(
			&lane.snapshot,
			&lane.review_state,
	...
	I: PullRequestReviewStateInspector,
{
	...
	let local_branch_name = match super::worktree_checkout_branch_name(worktree.worktree_path()) {
	...
	let local_head_oid = match super::worktree_head_oid(worktree.worktree_path()) {
...
{
	let review_state =
		match super::load_post_review_lane_review_state(snapshot, review_state_inspector)? {
...
mod tests {
	use crate::orchestrator::retained_review_orchestration::{
		self, CommandFact, CommandIntent, CommandIntentKind,
	};

	#[test]
	fn retained_review_command_adapter_accepts_kernel_built_marker_sync_intent() {
		let intent = retained_review_orchestration::retained_review_command_intent_for_issue(
			"PUB-101",
			Some("run-1"),
			CommandIntentKind::SyncReviewOrchestrationMarker,
			"review_orchestration_marker_created",
		);
		let accepted = retained_review_orchestration::retained_review_command_adapter(
			intent,
			CommandIntentKind::SyncReviewOrchestrationMarker,
		)
	...
	fn retained_review_command_adapter_rejects_intent_without_kernel_contract() {
		...
		let error = retained_review_orchestration::retained_review_command_adapter(
			intent,
			CommandIntentKind::StartRetainedLanding,
		)
