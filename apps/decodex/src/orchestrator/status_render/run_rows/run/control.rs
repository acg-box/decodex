...
pub(in crate::orchestrator::status_render::run_rows::run) fn render_lane_control_conditions(
...
pub(in crate::orchestrator::status_render::run_rows::run) fn render_optional_bool(
	value: Option<bool>,
) -> String {
	value.map_or_else(|| String::from("none"), |value| if value { "yes" } else { "no" }.into())
}

pub(in crate::orchestrator::status_render::run_rows::run) fn operator_run_queue_lease_summary(
	run: &OperatorRunStatus,
) -> String {
	if run.run_lease {
