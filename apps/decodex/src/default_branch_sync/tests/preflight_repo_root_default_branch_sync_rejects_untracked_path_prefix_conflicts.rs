...
fn preflight_repo_root_default_branch_sync_rejects_untracked_path_prefix_conflicts() {
	...
	fs::create_dir_all(peer_root.join("notes")).expect("peer nested directory should exist");
	fs::write(peer_root.join("notes/guide.md"), "remote tracked file\n")
		.expect("peer nested file should write");
	tests::run_git(&peer_root, &["add", "notes/guide.md"]);
	tests::run_git(&peer_root, &["commit", "-m", "add nested file"]);
	tests::run_git(&peer_root, &["push", "origin", "main"]);
	fs::write(repo_root.join("notes"), "local untracked file\n")
