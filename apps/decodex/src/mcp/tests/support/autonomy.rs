...
pub(in crate::mcp::tests) fn seed_autonomy_challenged_proposal() -> (TempDir, PathBuf, String) {
	...
	let proposal_call = format!(
		r#"{{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{{"name":"autonomy_compile_proposal","arguments":{{"mode":"apply","signalIds":["{signal_id}"],"proposal":{{"objectiveId":"quality-autonomy","objectiveVersion":1,"sourceFamily":"runtime_health","intendedSurface":"apps/decodex/src/mcp.rs","summary":"Expose autonomy MCP surface.","challengeRequirements":["independent challenge"],"rollbackPath":"Revert MCP autonomy surface.","issueCandidates":[{{"key":"mcp-runtime","title":"Implement the MCP runtime change","objective":"Implement the bounded accepted change.","stage":"runtime","dependencies":[],"conflictDomains":["module:mcp"],"acceptance":["MCP runtime behavior is verified."],"validation":["cargo test -p decodex mcp --lib"],"risk":["Preserve authority boundaries."],"queueIntent":"ready_to_queue"}}]}},"authority":{{"source":"mcp-test","reason":"compile proposal evidence"}}}}}}}}"#
	);
