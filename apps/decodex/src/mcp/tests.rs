...
fn observability_projection_resources_expose_activity_without_private_payloads() {
	...
	let events =
		super::mcp_run_resource(&snapshot, "run-1", "events").expect("run events should project");
...
fn mcp_review_surfaces_ignore_null_loop_review_status() {
	...
	let post_review_lane = super::mcp_public_post_review_lane(&snapshot["post_review_lanes"][0]);
...
fn http_delete<'a>(path: &str, headers: impl IntoIterator<Item = (&'a str, &'a str)>) -> Vec<u8> {
	let mut request =
...
fn http_options<'a>(path: &str, headers: impl IntoIterator<Item = (&'a str, &'a str)>) -> Vec<u8> {
	let mut request =
