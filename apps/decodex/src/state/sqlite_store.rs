...
impl SqliteStateStore {
	...
	pub(super) fn bootstrap_schema(&self) -> Result<()> {
...
"#,
	...
	pub(super) fn bootstrap_worktree_schema(&self) -> Result<()> {
		self.ensure_column(
			"worktrees",
			"provenance_source",
	...
	pub(super) fn bootstrap_review_schema(&self) -> Result<()> {
	...
	pub(super) fn bootstrap_evidence_artifact_schema(&self) -> Result<()> {
	...
	pub(super) fn bootstrap_run_control_channels_schema(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn bootstrap_loop_guardrail_schema(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn bootstrap_connector_backoffs_schema(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn bootstrap_private_execution_events_schema(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn bootstrap_decision_contracts_schema(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn bootstrap_autonomy_objectives_schema(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn bootstrap_autonomy_signals_schema(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn bootstrap_autonomy_proposals_schema(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn bootstrap_execution_programs_schema(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn ensure_execution_program_source_contract_nullable(&self) -> Result<()> {
...
"#,
	...
	pub(super) fn bootstrap_program_intake_state_schema(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn backfill_program_intake_state_from_execution_programs(&self) -> Result<()> {
	...
	pub(super) fn schema_version(&self) -> Result<Option<i64>> {
	...
	pub(super) fn ensure_schema_meta_table(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
...
"#,
	...
	pub(super) fn schema_migration_completed(&self, key: &str) -> Result<bool> {
		self.ensure_schema_meta_table()?;
		let value = self
			.connection
	...
	pub(super) fn record_schema_migration_completed(&self, key: &str) -> Result<()> {
		self.ensure_schema_meta_table()?;
	...
	pub(super) fn run_schema_migrations(&self) -> Result<()> {
	...
	pub(super) fn record_schema_version(&self) -> Result<()> {
		self.connection.execute_batch(
			r#"
CREATE TABLE IF NOT EXISTS schema_meta (
	key TEXT PRIMARY KEY NOT NULL,
...
"#,
	...
	pub(super) fn backfill_protocol_event_summaries_from_events(&self) -> Result<()> {
	...
	pub(super) fn migrate_legacy_decision_contract_issue_summaries(&self) -> Result<()> {
		let updates = {
			let mut statement = self.connection.prepare(
	...
	pub(super) fn seal_run_activity_summary_records(&self) -> Result<()> {
		let updates = {
			let mut statement = self.connection.prepare(
	...
	pub(super) fn load_state(&self) -> Result<StateData> {
	...
	pub(super) fn load_project_run_metadata_for_project(
		&self,
		project_id: &str,
	) -> Result<StateData> {
		let mut state = StateData::default();
	...
	pub(super) fn load_project_loop_evidence_for_project(
		&self,
		project_id: &str,
	) -> Result<StateData> {
		let mut state = StateData::default();
	...
	pub(super) fn load_project_registry_state(&self) -> Result<StateData> {
		let mut state = StateData::default();
	...
	pub(super) fn persist_runtime_state(&mut self, state: &StateData) -> Result<()> {
	...
	pub(super) fn delete_project(&mut self, service_id: &str) -> Result<()> {
		let transaction = self.connection.transaction()?;
	...
	pub(super) fn upsert_project(&self, project: &ProjectRegistration) -> Result<()> {
		self.connection.execute(
	...
	pub(super) fn delete_connector_backoff(&self, project_id: &str, connector: &str) -> Result<()> {
		self.connection.execute(
	...
	pub(super) fn connector_backoff(
		&self,
		project_id: &str,
		connector: &str,
	...
	pub(super) fn upsert_run_attempt(&self, attempt: &RunAttemptRecord) -> Result<()> {
		self.connection.execute(
	...
	pub(super) fn upsert_run_control_channel(
		&self,
	...
	pub(super) fn upsert_run_activity_summary(
		&self,
	...
	pub(super) fn upsert_lease_and_remember_run_project(
		&mut self,
		lease: &IssueLease,
	) -> Result<()> {
		let transaction = self.connection.transaction()?;
	...
	pub(super) fn upsert_worktree_and_remember_run_project(
		&mut self,
	...
	pub(super) fn insert_linear_execution_event_if_absent(
		&self,
	...
	pub(super) fn delete_linear_execution_event(&self, idempotency_key: &str) -> Result<()> {
		self.connection.execute(
	...
	pub(super) fn insert_private_execution_event(
		&self,
	...
	#[allow(dead_code)]
	pub(super) fn upsert_decision_contract(
		&self,
	...
	#[allow(dead_code)]
	pub(super) fn upsert_autonomy_objective(
		&self,
	...
	pub(super) fn upsert_autonomy_signal(
		&self,
	...
	pub(super) fn upsert_autonomy_proposal(
		&self,
	...
	#[allow(dead_code)]
	pub(super) fn upsert_execution_program(
		&self,
	...
	pub(super) fn replace_program_intake_state(
		&self,
		record: &ExecutionProgramRuntimeRecord,
	) -> Result<()> {
		self.connection.execute(
	...
	pub(super) fn delete_lease(&mut self, issue_id: &str) -> Result<()> {
		self.connection.execute("DELETE FROM leases WHERE issue_id = ?1", params![issue_id])?;

		Ok(())
	}

	pub(super) fn retarget_issue_identity(
		&mut self,
	...
	pub(super) fn delete_worktree_and_review_lifecycle(&mut self, issue_id: &str) -> Result<()> {
		let transaction = self.connection.transaction()?;
	...
	pub(super) fn delete_worktree_mapping(&mut self, issue_id: &str) -> Result<()> {
		self.connection.execute("DELETE FROM worktrees WHERE issue_id = ?1", params![issue_id])?;

		Ok(())
	}

	pub(super) fn delete_review_marker_identity(
		&mut self,
		project_id: &str,
		issue_id: &str,
		branch_name: &str,
	...
	pub(super) fn delete_loop_guardrail_checkpoints_for_issue(
		&mut self,
		project_id: &str,
		issue_id: &str,
	) -> Result<()> {
	...
	pub(super) fn delete_loop_guardrail_checkpoint(
		&mut self,
		project_id: &str,
		issue_id: &str,
		reason: &str,
	...
	pub(super) fn delete_review_policy_checkpoints_for_run_attempt(
		&mut self,
		project_id: &str,
		issue_id: &str,
		run_id: &str,
	...
	pub(super) fn load_projects(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_leases(&self, state: &mut StateData) -> Result<()> {
	...
	pub(super) fn load_run_attempts(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_run_attempts_for_project(
		&self,
		state: &mut StateData,
		project_id: &str,
	) -> Result<()> {
	...
	pub(super) fn load_run_control_channels(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_run_control_channels_for_project(
		&self,
		state: &mut StateData,
		project_id: &str,
	) -> Result<()> {
	...
	pub(super) fn retry_budget_attempt_count(&self, issue_id: &str) -> Result<i64> {
		self.connection
			.query_row(
	...
	pub(super) fn issue_has_retry_budget_attempt_after(
		&self,
		issue_id: &str,
		attempt_number: i64,
	) -> Result<bool> {
	...
	pub(super) fn run_attempt_for_issue_attempt(
		&self,
		issue_id: &str,
		attempt_number: i64,
	...
	pub(super) fn latest_run_attempt_for_issue(
		&self,
		issue_id: &str,
	) -> Result<Option<RunAttemptRecord>> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn list_run_attempts_for_issue(
		&self,
		issue_id: &str,
	...
	pub(super) fn list_run_attempts_for_project(
		&self,
		project_id: &str,
	) -> Result<Vec<RunAttemptRecord>> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn run_has_protocol_event(&self, run_id: &str, event_type: &str) -> Result<bool> {
	...
	pub(super) fn load_protocol_event_summaries(&self, state: &mut StateData) -> Result<()> {
		self.load_compacted_protocol_event_summaries(state)
	}

	pub(super) fn load_protocol_event_summaries_for_runs(
		&self,
		state: &mut StateData,
		run_ids: &[String],
	) -> Result<()> {
	...
	pub(super) fn rebuild_protocol_event_summaries_for_runs(
		&self,
		state: &mut StateData,
		run_ids: &[String],
	) -> Result<()> {
	...
	pub(super) fn load_protocol_event_summary_for_run(
		&self,
		state: &mut StateData,
		run_id: &str,
	) -> Result<()> {
	...
	pub(super) fn load_run_activity_summaries(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_run_activity_summaries_for_loaded_runs(
		&self,
		state: &mut StateData,
	) -> Result<()> {
	...
	pub(super) fn load_run_activity_summaries_for_runs(
		&self,
		state: &mut StateData,
		run_ids: &[String],
	) -> Result<()> {
	...
	pub(super) fn load_run_activity_summary_for_run(
		&self,
		state: &mut StateData,
		run_id: &str,
	) -> Result<()> {
	...
	pub(super) fn load_compacted_protocol_event_summaries(
		&self,
		state: &mut StateData,
	) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_compacted_protocol_event_summary_for_run(
		&self,
		state: &mut StateData,
		run_id: &str,
	) -> Result<bool> {
	...
	pub(super) fn upsert_protocol_event_summary(
		&self,
		run_id: &str,
	...
	pub(super) fn load_worktrees(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn worktree_for_issue(
		&self,
		issue_id: &str,
	...
	pub(super) fn load_linear_execution_events(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn list_linear_execution_events(
		&self,
		service_id: &str,
		issue_id: &str,
	...
	pub(super) fn load_private_execution_events(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_private_execution_events_for_project(
		&self,
		state: &mut StateData,
		project_id: &str,
	) -> Result<()> {
	...
	pub(super) fn load_decision_contracts(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn decision_contract(
		&self,
		project_id: &str,
		contract_id: &str,
	...
	pub(super) fn decision_contract_for_readback(
		&self,
		project_id: &str,
		contract_id: &str,
	) -> Result<Option<DecisionContractRuntimeRecord>> {
	...
	pub(super) fn list_decision_contracts_for_issue(
		&self,
		project_id: &str,
		source_issue_id: &str,
	...
	pub(super) fn list_decision_contracts_for_project(
		&self,
		project_id: &str,
	) -> Result<Vec<DecisionContractRuntimeRecord>> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_autonomy_objectives(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn autonomy_objective(
		&self,
		project_id: &str,
		objective_id: &str,
		version: u64,
	...
	pub(super) fn current_accepted_autonomy_objective(
		&self,
		project_id: &str,
		objective_id: &str,
	) -> Result<Option<AutonomyObjectiveRuntimeRecord>> {
	...
	pub(super) fn list_autonomy_objective_history(
		&self,
		project_id: &str,
		objective_id: &str,
	...
	pub(super) fn recent_autonomy_objectives_for_project(
		&self,
		project_id: &str,
		limit: usize,
	) -> Result<Vec<AutonomyObjectiveRuntimeRecord>> {
	...
	pub(super) fn load_autonomy_signals(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_autonomy_signals_for_project(
		&self,
		state: &mut StateData,
		project_id: &str,
	) -> Result<()> {
	...
	pub(super) fn list_autonomy_signals_for_objective(
		&self,
		project_id: &str,
		objective_id: &str,
	...
	pub(super) fn recent_autonomy_signals_for_project(
		&self,
		project_id: &str,
		limit: usize,
	) -> Result<Vec<AutonomySignalRuntimeRecord>> {
	...
	pub(super) fn load_autonomy_proposals(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_autonomy_proposals_for_project(
		&self,
		state: &mut StateData,
		project_id: &str,
	) -> Result<()> {
	...
	pub(super) fn autonomy_proposal(
		&self,
		project_id: &str,
		proposal_id: &str,
	...
	pub(super) fn list_autonomy_proposals_for_objective(
		&self,
		project_id: &str,
		objective_id: &str,
		objective_version: u64,
	...
	pub(super) fn recent_autonomy_proposals_for_project(
		&self,
		project_id: &str,
		limit: usize,
	) -> Result<Vec<AutonomyProposalRuntimeRecord>> {
	...
	pub(super) fn load_execution_programs(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
			"SELECT project_id, program_id, source_contract_id, payload_json, created_at, \
			 created_at_unix, updated_at, updated_at_unix \
			 FROM execution_programs \
	...
	pub(super) fn execution_program(
		&self,
		project_id: &str,
		program_id: &str,
	...
	pub(super) fn list_execution_programs_for_contract(
		&self,
		project_id: &str,
	...
	pub(super) fn list_execution_programs(
		&self,
		project_id: &str,
	) -> Result<Vec<ExecutionProgramRuntimeRecord>> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_program_intake_state(&self, state: &mut StateData) -> Result<()> {
	...
	pub(super) fn list_all_program_intake_plans(&self) -> Result<Vec<ProgramIntakePlanRecord>> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn list_program_intake_plans(
		&self,
		project_id: &str,
	...
	pub(super) fn list_all_program_issue_mappings(&self) -> Result<Vec<ProgramIssueMappingRecord>> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn list_program_issue_mappings(
		&self,
		project_id: &str,
		program_id: &str,
	...
	pub(super) fn load_review_lifecycle_records(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_review_lifecycle_records_for_project(
		&self,
		state: &mut StateData,
		project_id: &str,
	) -> Result<()> {
	...
	pub(super) fn load_review_policy_checkpoints(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_review_policy_checkpoints_for_project(
		&self,
		state: &mut StateData,
		project_id: &str,
	) -> Result<()> {
	...
	pub(super) fn load_evidence_artifacts(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_evidence_artifacts_for_project(
		&self,
		state: &mut StateData,
		project_id: &str,
	) -> Result<()> {
	...
	pub(super) fn evidence_artifact_from_row(
		row: &Row<'_>,
	...
	pub(super) fn load_loop_guardrail_checkpoints(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
	...
	pub(super) fn load_connector_backoffs(&self, state: &mut StateData) -> Result<()> {
		let mut statement = self.connection.prepare(
			"SELECT project_id, connector, sync_phase, quota_class, reset_unix_epoch, \
