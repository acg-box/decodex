...
"#,
	...
	fn bootstrap_review_schema(&self) -> Result<()> {
		let legacy_handoffs_exist = self.table_exists("review_handoffs")?;
		let legacy_orchestrations_exist = self.table_exists("review_orchestrations")?;

		self.connection.execute_batch(REVIEW_LIFECYCLE_SCHEMA_SQL)?;

		match (legacy_handoffs_exist, legacy_orchestrations_exist) {
			(true, true) => {
				self.migrate_legacy_review_handoffs_with_orchestrations()?;
				self.migrate_legacy_review_orchestration_orphans()?;
			},
			(true, false) => self.migrate_legacy_review_handoffs()?,
			(false, true) => self.migrate_legacy_review_orchestrations()?,
			(false, false) => {},
		}
	...
	fn migrate_legacy_review_handoffs(&self) -> Result<()> {
		self.connection.execute(
			"INSERT OR IGNORE INTO review_lifecycle_records (
			 ...
			 SELECT project_id, issue_id, branch_name, run_id, attempt_number, pr_url,
					target_base_ref_name, pr_head_ref_name, pr_head_oid, pr_head_oid,
					'request_pending', NULL, NULL, NULL, 0, 0, NULL, 'not_started',
					'not_started', 0, '{}', '', updated_at, updated_at_unix
			 FROM review_handoffs
			 ORDER BY updated_at_unix DESC, updated_at DESC",
			[],
		)?;
	...
	fn migrate_legacy_review_orchestrations(&self) -> Result<()> {
		self.connection.execute(
			"INSERT OR IGNORE INTO review_lifecycle_records (
					project_id, issue_id, branch_name, run_id, attempt_number, pr_url,
					target_base_ref_name, pr_head_ref_name, pr_head_oid, head_sha, phase,
			 ...
			 SELECT project_id, issue_id, branch_name, run_id, attempt_number, pr_url,
					NULL, branch_name, head_sha, head_sha, phase,
					request_comment_database_id, request_created_at_unix_epoch,
					request_description_thumbs_up_count, request_retry_count, external_round_count,
					auto_merge_enabled_at_unix_epoch, 'not_started', 'not_started',
					0, '{}', '', updated_at, updated_at_unix
	...
	fn migrate_legacy_review_handoffs_with_orchestrations(&self) -> Result<()> {
		self.connection.execute(
			"INSERT OR IGNORE INTO review_lifecycle_records (
					project_id, issue_id, branch_name, run_id, attempt_number, pr_url,
					target_base_ref_name, pr_head_ref_name, pr_head_oid, head_sha, phase,
			 ...
			 SELECT h.project_id, h.issue_id, h.branch_name, h.run_id, h.attempt_number, h.pr_url,
					h.target_base_ref_name, h.pr_head_ref_name, h.pr_head_oid,
					COALESCE(o.head_sha, h.pr_head_oid), COALESCE(o.phase, 'request_pending'),
					o.request_comment_database_id, o.request_created_at_unix_epoch,
					o.request_description_thumbs_up_count, COALESCE(o.request_retry_count, 0),
					COALESCE(o.external_round_count, 0), o.auto_merge_enabled_at_unix_epoch,
					'not_started', 'not_started', 0, '{}', '',
					COALESCE(o.updated_at, h.updated_at),
					COALESCE(o.updated_at_unix, h.updated_at_unix)
			 FROM review_handoffs AS h
			 LEFT JOIN review_orchestrations AS o
			   ON o.project_id = h.project_id
			  AND o.issue_id = h.issue_id
			  AND o.branch_name = h.branch_name
			  AND o.run_id = h.run_id
			  AND o.attempt_number = h.attempt_number
			 ORDER BY h.updated_at_unix DESC, h.updated_at DESC",
			[],
		)?;
	...
	fn migrate_legacy_review_orchestration_orphans(&self) -> Result<()> {
		self.connection.execute(
			"INSERT OR IGNORE INTO review_lifecycle_records (
					project_id, issue_id, branch_name, run_id, attempt_number, pr_url,
					target_base_ref_name, pr_head_ref_name, pr_head_oid, head_sha, phase,
			 ...
			 SELECT o.project_id, o.issue_id, o.branch_name, o.run_id, o.attempt_number, o.pr_url,
					NULL, o.branch_name, o.head_sha, o.head_sha, o.phase,
					o.request_comment_database_id, o.request_created_at_unix_epoch,
					o.request_description_thumbs_up_count, o.request_retry_count,
					o.external_round_count, o.auto_merge_enabled_at_unix_epoch,
					'not_started', 'not_started', 0, '{}', '', o.updated_at, o.updated_at_unix
			 FROM review_orchestrations AS o
			 WHERE NOT EXISTS (
				 SELECT 1
				 FROM review_handoffs AS h
				 WHERE h.project_id = o.project_id
				   AND h.issue_id = o.issue_id
				   AND h.branch_name = o.branch_name
			 )
			 ORDER BY o.updated_at_unix DESC, o.updated_at DESC",
			[],
		)?;
