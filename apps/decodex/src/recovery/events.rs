...
use super::{
	AdoptValidation, LEGACY_MANUAL_CLOSEOUT_ANCHOR, LEGACY_MANUAL_CLOSEOUT_EVENT,
	MERGED_CLOSEOUT_CLEANUP_ANCHOR, MERGED_CLOSEOUT_CLOSEOUT_ANCHOR, MergedCloseoutValidation,
	REVIEW_HANDOFF_ADOPT_EVENT, REVIEW_HANDOFF_REBIND_EVENT, RebindValidation, RecoveryContext,
	landing_url,
};
...
pub(super) fn legacy_closeout_event(
	context: &RecoveryContext,
	validation: &super::LegacyCloseoutValidation,
) -> LinearExecutionEventRecord {
	let pr_url = landing_url(&validation.landing_state);
	let stable_anchor = records::stable_event_anchor(&[
		pr_url,
