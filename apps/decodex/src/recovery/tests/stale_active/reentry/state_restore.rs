...
use crate::{
	recovery::{
		...
		RecoveryRuntimeMutationPolicy, STALE_ACTIVE_STATE_RESTORE_CLASSIFICATION,
		tests::{
...
fn stale_active_release_reentry_restores_startable_state_after_active_label_release() {
	...
	let mut issue = tests::sample_issue_with_labels("In Progress", &[queue_label]);
	...
	assert_eq!(diagnostic.classification, STALE_ACTIVE_STATE_RESTORE_CLASSIFICATION);
