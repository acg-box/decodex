...
#[test]
fn manual_land_closeout_receipt_roundtrips() {
	...
	let marker = manual::read_manual_land_closeout_receipt(&checkout)
		.expect("closeout marker should parse")
		.expect("closeout marker should exist");

	assert_eq!(marker.landed_change.as_deref(), Some(r#"{"schema":"decodex/commit/2"}"#));
	assert!(
