...
material-specific variations. Squares share one material group but retain individual layer effects: inside
specular highlights, 16% refraction depth, 10% refraction strength, and neutral
shadows at 22% (Default), 30% (Dark), and 26% (Mono). These native effects give the
pixels thin edges and depth while retaining the per-cell color progression. The menu template uses the same 15 cells without omitting or moving any. Cells
