...
from contracts import load_json, validate_bundle  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument("paths", nargs="+", help="Bundle JSON files or directories.")
    args = parser.parse_args()

    errors: list[str] = []
    for path in iter_json_files(args.paths):
        result = validate_bundle(load_json(path))
        if result.ok:
            continue
