...
pub struct ChiefPermissionAttempt {
	pub work: String,
	pub thread: String,
...
pub struct ChiefPermissionReceipt {
	pub id: i64,
	pub attempt: ChiefPermissionAttempt,
	pub state: String,
}

impl ChiefPermissionAttempt {
...
pub(crate) fn pending(connection: &rusqlite::Connection, work: &str) -> Result<bool, StoreError> {
	connection.query_row("SELECT EXISTS(SELECT 1 FROM chief_inbox_events e WHERE e.work_item_id=?1 AND e.event_kind='permission_selection' AND NOT EXISTS(SELECT 1 FROM chief_inbox_events r WHERE (r.source_event_id=e.source_event_id||':result' AND r.event_kind='permission_selection_result' AND json_extract(r.payload,'$.state')='rejected') OR (r.source_event_id=e.source_event_id||':observation' AND r.event_kind='permission_selection_observation')))", [work], |r|r.get(0)).map_err(|e|sqlite_error(e).into())
}
...
impl SqliteStore {
	...
	pub async fn reserve_chief_permission_selection(
		&self,
		attempt: ChiefPermissionAttempt,
	) -> Result<Option<i64>, StoreError> {
		attempt.validate()?;
		self.run(move |connection| {
			let tx=connection.transaction_with_behavior(TransactionBehavior::Immediate).map_err(sqlite_error)?;
			if !owns_work(&tx,&attempt.work,attempt.generation.as_deref())? || crate::chief_prompt_edit::pending(&tx,&attempt.work)? || pending(&tx,&attempt.work)? || crate::chief_models::pending(&tx,&attempt.work)? || crate::chief_plugins::pending(&tx,&attempt.work)? { return Ok(None); }
	...
	pub async fn finish_chief_permission_selection(
		&self,
		event: i64,
		attempt: ChiefPermissionAttempt,
		state: String,
		...
		self.run(move |connection| {
			...
			Ok(connection.execute("INSERT OR IGNORE INTO chief_inbox_events(source_event_id,work_item_id,event_kind,payload,created_at_micros,disposition,disposition_note,disposed_at_micros) SELECT source_event_id||':result',work_item_id,'permission_selection_result',?4,?5,'resolved','Native RPC outcome; application is separate.',?5 FROM chief_inbox_events WHERE id=?1 AND source_event_id=?2 AND payload=?3 AND event_kind='permission_selection'",params![event,key,expected,json!({"reservation":event,"state":state}).to_string(),unix_micros()?]).map_err(sqlite_error)?==1)
		}).await
	}

	pub async fn chief_permission_receipt(
		&self,
		work: String,
		thread: String,
	) -> Result<Option<ChiefPermissionReceipt>, StoreError> {
		self.run(move |connection| {
			let row:Option<(i64,String,String)>=connection.query_row("SELECT e.id,json_extract(e.payload,'$.attempt'),COALESCE(json_extract(o.payload,'$.state'),json_extract(r.payload,'$.state'),'reserved') FROM chief_inbox_events e LEFT JOIN chief_inbox_events r ON r.source_event_id=e.source_event_id||':result' AND r.event_kind='permission_selection_result' LEFT JOIN chief_inbox_events o ON o.source_event_id=e.source_event_id||':observation' AND o.event_kind='permission_selection_observation' WHERE e.work_item_id=?1 AND e.event_kind='permission_selection' AND json_extract(e.payload,'$.attempt.thread')=?2 ORDER BY e.id DESC LIMIT 1",params![work,thread],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?))).optional().map_err(sqlite_error)?;
			row.map(|(id,attempt,state)|Ok(ChiefPermissionReceipt {id,attempt:serde_json::from_str(&attempt).map_err(|_|StoreError::InvalidInput("invalid saved permission attempt"))?,state})).transpose()
		}).await
	}
}
...
mod tests {
	...
	async fn facts(store: &SqliteStore, profile: Option<&str>, digest: char) -> i64 {
		store.record_chief_task_permissions_publication("thread".into(),None,profile.map(|profile|json!({"profileId":profile,"cwd":"/native","approvalPolicy":"on-request","approvalsReviewer":"user","sandboxPolicy":{"type":"readOnly"}}).to_string()),digest.to_string().repeat(64)).await.unwrap().unwrap()
	}
	fn attempt(settings_event: i64, token: char) -> ChiefPermissionAttempt {
		ChiefPermissionAttempt {
			work: "work".into(),
			thread: "thread".into(),
			generation: None,
			settings_event,
	...
	async fn receipt(store: &SqliteStore) -> ChiefPermissionReceipt {
		store.chief_permission_receipt("work".into(), "thread".into()).await.unwrap().unwrap()
	}
	...
	async fn running_permission_reservation_accepts_named_profiles_and_does_not_replay() {
		...
		store.begin_chief_dispatch("work".into()).await.expect("dispatch");
		let selected = attempt(observed, 'a');
		assert!(
			store
				.reserve_chief_permission_selection(selected.clone())
				.await
				.expect("reserve")
				.is_none()
		);
		store.acknowledge_chief_dispatch("work".into(), "active".into()).await.expect("running");
		let id = store
			.reserve_chief_permission_selection(selected.clone())
			.await
			...
			.expect("reserved");
		store
			.finish_chief_permission_selection(id, selected.clone(), "unknown".into())
			.await
			.expect("uncertain");
		drop(store);
		...
		assert!(
			reopened.reserve_chief_permission_selection(selected).await.expect("replay").is_none()
		);
		facts(&reopened, Some("scoped"), 'c').await;
		assert_eq!(receipt(&reopened).await.state, "target_observed");
		assert_eq!(
			reopened.get_chief_work_item("work".into()).await.expect("work").dispatch_state,
	...
	async fn permission_reservation_survives_crash_and_blocks_dispatch_until_native_confirmation() {
		...
		let (one, two) = tokio::join!(
			store.reserve_chief_permission_selection(a.clone()),
			store.reserve_chief_permission_selection(b.clone())
		);
		...
		assert!(
			store
				.reserve_chief_permission_selection(attempt(observed, 'c'))
				.await
				.unwrap()
				.is_none()
		);
		assert!(
			store
				.finish_chief_permission_selection(id, winning.clone(), "unknown".into())
				.await
				.unwrap()
		);
		assert!(
			!store.finish_chief_permission_selection(id, winning, "queued".into()).await.unwrap()
		);
	...
	async fn permission_publication_before_ack_wins_and_rejected_review_cannot_replay() {
		...
		assert!(
			store.reserve_chief_permission_selection(attempt(first, 'a')).await.unwrap().is_none()
		);
		let a = attempt(newer, 'a');
		let id = store.reserve_chief_permission_selection(a.clone()).await.unwrap().unwrap();
		let mut forged = a.clone();
		forged.profile = "different".into();
		assert!(
			!store.finish_chief_permission_selection(id, forged, "rejected".into()).await.unwrap()
		);
		facts(&store, Some("scoped"), 'c').await;
		assert!(
			store.finish_chief_permission_selection(id, a.clone(), "queued".into()).await.unwrap()
		);
		assert_eq!(receipt(&store).await.state, "target_observed");
		...
		assert!(store.reserve_chief_permission_selection(replay).await.unwrap().is_none());
		let next = attempt(reverted, 'b');
		let next_id =
			store.reserve_chief_permission_selection(next.clone()).await.unwrap().unwrap();
		assert!(
			store
				.finish_chief_permission_selection(next_id, next.clone(), "rejected".into())
				.await
				.unwrap()
		);
		facts(&store, Some("scoped"), 'e').await;
		assert_eq!(receipt(&store).await.state, "rejected");
		assert!(store.reserve_chief_permission_selection(next).await.unwrap().is_none());
		assert!(store.begin_chief_dispatch("work".into()).await.is_ok());
	}
	#[tokio::test]
	async fn observation_revisions_preserve_reversions_without_consuming_transcript_pages() {
		...
		assert_eq!(
			store
				.chief_task_permissions("work".into(), "thread".into(), None)
				.await
				.unwrap()
				.unwrap()
				.id,
		...
		assert!(
			store
				.chief_task_permissions("foreign".into(), "thread".into(), None)
				.await
				.unwrap()
				.is_none()
		);
		...
		let reserved =
			store.reserve_chief_permission_selection(attempt.clone()).await.unwrap().unwrap();
		store.finish_chief_permission_selection(reserved, attempt, "queued".into()).await.unwrap();
