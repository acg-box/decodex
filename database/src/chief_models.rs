...
impl SqliteStore {
	...
	pub async fn reserve_chief_model_selection(
		...
		self.run(move |connection| {
			...
			if !owns_work(&tx,&attempt.work,attempt.generation.as_deref())? || pending(&tx,&attempt.work)? || crate::chief_permissions::pending(&tx,&attempt.work)? || crate::chief_plugins::pending(&tx,&attempt.work)? { return Ok(None); }
