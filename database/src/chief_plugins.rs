...
impl SqliteStore {
	...
	pub async fn reserve_chief_plugin_selection(
		...
		self.run(move |connection| {
			...
			if !owns_work(&tx,&attempt.work,attempt.generation.as_deref())? || pending(&tx,&attempt.work)? || crate::chief_models::pending(&tx,&attempt.work)? || crate::chief_permissions::pending(&tx,&attempt.work)? { return Ok(None); }
