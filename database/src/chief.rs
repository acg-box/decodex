...
impl SqliteStore {
	...
	pub async fn read_chief_work_events_before(
		...
		self.run(move |connection| {
			...
			connection.prepare("SELECT * FROM (SELECT * FROM chief_inbox_events WHERE work_item_id = ?1 AND event_kind NOT IN ('turn_execution','native_task_settings','token_usage','response_usage','live_reviewer_attempt','live_reviewer_result') AND (?3 IS NULL OR id < ?3) ORDER BY id DESC LIMIT ?2) ORDER BY id")
