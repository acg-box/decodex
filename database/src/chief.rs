...
impl SqliteStore {
	...
	pub async fn read_chief_work_events_before(
		...
		self.run(move |connection| {
			...
			connection.prepare("SELECT * FROM (SELECT * FROM chief_inbox_events WHERE work_item_id = ?1 AND event_kind NOT IN ('reasoning_voice_handoff','turn_execution','native_task_settings','token_usage','response_usage','live_reviewer_attempt','live_reviewer_result','native_task_permissions','permission_selection','permission_selection_result','permission_selection_observation','native_task_models','model_selection','model_selection_result','model_selection_observation','native_task_plugins','plugin_selection','plugin_selection_result','plugin_selection_observation','hook_setting_attempt','hook_setting_result','hook_setting_observation','app_setting_attempt','app_setting_result','app_setting_observation') AND (?3 IS NULL OR id < ?3) ORDER BY id DESC LIMIT ?2) ORDER BY id")
	...
	pub async fn read_chief_transcript(
		...
		self.run(move |connection| {
            ...
            read_work(&tx,&id)?;
            let events=tx.prepare("SELECT * FROM (SELECT e.* FROM chief_inbox_events e WHERE work_item_id=?1 AND event_kind NOT IN ('reasoning_voice_handoff','turn_execution','native_task_settings','token_usage','response_usage','live_reviewer_attempt','live_reviewer_result','native_task_permissions','permission_selection','permission_selection_result','permission_selection_observation','native_task_models','model_selection','model_selection_result','model_selection_observation','native_task_plugins','plugin_selection','plugin_selection_result','plugin_selection_observation','hook_setting_attempt','hook_setting_result','hook_setting_observation','app_setting_attempt','app_setting_result','app_setting_observation') AND (event_kind<>'activity_started' OR NOT EXISTS(SELECT 1 FROM chief_inbox_events c WHERE c.source_event_id=json_array('activity',e.work_item_id,json_extract(e.payload,'$.turn_id'),json_extract(e.payload,'$.item_id'),'completed'))) AND (event_kind<>'steer_pending' OR disposition IS NULL) AND (?3 IS NULL OR id<?3) ORDER BY id DESC LIMIT ?2) ORDER BY id").map_err(sqlite_error)?.query_map(params![id,limit,before],event_row).map_err(sqlite_error)?.collect::<Result<Vec<_>,_>>().map_err(sqlite_error)?;
	...
	pub async fn begin_chief_dispatch_with_input(
		...
		self.run(move |connection| {
			let transaction = connection.transaction_with_behavior(TransactionBehavior::Immediate).map_err(sqlite_error)?;
			let work = read_work(&transaction, &id)?;
			if work.dispatch_state != ChiefDispatchState::Idle || work.codex_thread_id.is_none() || (crate::chief_permissions::pending(&transaction, &id)? || crate::chief_plugins::pending(&transaction, &id)? || crate::chief_models::pending(&transaction, &id)?) {
