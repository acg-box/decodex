...
impl SqliteStore {
	/// Observe a weekly window and reserve at most one activation before any network effect.
	...
	pub async fn claim_quota_activation(
		...
		self.run(move |connection| {
			...
			let prior: Option<(i64, i64, Option<i64>)> = tx
				.query_row(
					"SELECT observed_reset_at_micros,next_due_at_micros,attempted_at_micros
			...
			let (reset, mut due) = match prior {
				Some((previous_reset, due, _)) if reported_reset <= previous_reset =>
					(previous_reset, due),
				_ => (reported_reset, reported_reset),
			};
			let claim = can_send && reported_reset == reset && reset <= now && due <= now;
			if claim {
				// Only a newer provider reset can permit another attempt, even after restart.
				due = i64::MAX;
			}

			tx.execute(
				"INSERT INTO account_quota_activation
				 (account_id,observed_reset_at_micros,next_due_at_micros,attempted_at_micros,outcome)
				 VALUES (?1,?2,?3,?4,?5)
