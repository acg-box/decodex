...
async fn permission_recovery_requires_dead_old_process_and_current_complete_owner_facts() {
	for profile in [Some("scoped"), Some("different"), None] {
		...
		assert_eq!(
			store
				.chief_permission_receipt("root".into(), "task".into())
				.await
				.unwrap()
				.unwrap()
				.state,
		...
		assert!(
			!store
				.finish_chief_permission_selection(reserved, attempt, "queued".into())
				.await
				.unwrap()
		);
		drop(store);
...
async fn prepare_unknown_permission_selection(
	store: &SqliteStore,
	profile: Option<&str>,
) -> (i64, crate::ChiefPermissionAttempt) {
	seed(store).await;
	...
	let event = store
		...
		.unwrap()
		.unwrap();
	let attempt = crate::ChiefPermissionAttempt {
		work: "root".into(),
		thread: "task".into(),
	...
	assert!(
		store
			.finish_chief_permission_selection(reserved, attempt.clone(), "unknown".into())
			.await
			.unwrap()
	);
	store
	...
	assert_eq!(
		store.chief_permission_receipt("root".into(), "task".into()).await.unwrap().unwrap().state,
		"unknown"
	);
	(reserved, attempt)
}
