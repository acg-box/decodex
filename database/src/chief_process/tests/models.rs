...
#[tokio::test]
async fn model_recovery_requires_dead_old_process_and_current_complete_owner_facts() {
	for profile in [Some("scoped"), Some("different"), None] {
		...
		store
			...
			.unwrap();
		assert_eq!(
			store.chief_model_receipt("root".into(), "task".into()).await.unwrap().unwrap().state,
			"unknown"
		);
		store
			.record_chief_task_models_publication(
...
async fn prepare_unknown_model_selection(
	...
	assert!(
		store
			.finish_chief_model_selection(reserved, attempt.clone(), "unknown".into())
			.await
			.unwrap()
	);
	store
