...
3. Before composing on X, create the active `social_publish_reservation/v1` only through `cargo run -p radar -- social reserve-publish`; do not hand-write reservation JSON. The command must pass daily-cap, active-reservation, terminal-post, idempotency, and validation checks.
4. Use Chrome for X only after `cargo run -p radar -- social reserve-publish` returns a reserved path. Verify the logged-in account is `@decodexspace` before composing; any other account is a terminal blocked outcome, not a manual workaround.
...
7. Validate changed JSON with `cargo run -p radar -- validate`.
