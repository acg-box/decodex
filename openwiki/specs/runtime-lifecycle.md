...
Promotion requires explicit accepted authority. Accepting a proposal can create a normal latent Decision Contract candidate, but it does not create tracker issues, Program Intake rows, queue labels, worktrees, or execution lanes. Runtime-policy or external-agent proposal acceptance requires a resolved accepted project-policy record for the project, objective version, actor, acceptance source, and `autonomy_proposal_acceptance` scope; external-agent output cannot accept its own proposal without that policy authority.
