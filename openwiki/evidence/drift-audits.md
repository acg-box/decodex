...
Use reverse checks when changing MCP, OpenWiki MCP claims, README MCP claims, plugin skills, operator-control docs, or remote-control examples:

```sh
git grep -n -E "bearer|Authorization|allow-origin|capability-profile|streamable-http|Mcp-Session-Id|decodex_lane_control|decodex_project_control|manual_attention|retained_resume" -- README.md apps/decodex/src apps/decodex/tests plugins/decodex openwiki
